...
import convnext_pt.convnextv2 as pt_models
import convnext_tf.convnext_v2 as tf_models
...
model_urls = [
    ...
    'https://dl.fbaipublicfiles.com/convnext/convnextv2/im22k/convnextv2_huge_22k_224_ema.pt',
...
def main():
    ...
    for model_url in model_urls:
        ...
        save_name = file_name.replace('pt', 'h5')
        ...
        print(tf_models.__dict__[model_name])
        print(tf_model.count_params(), sum(p.numel() for p in pt_model.parameters()))
        ...
        for m in range(4):
            ...
            for i in range(num_blocks):
                ...
                for j, layer in enumerate(stage_block.layers):
                    ...
                    elif isinstance(layer, tf_models.GRN):
                        layer.gamma.assign(
                            tf.Variable(model_states[f'{stage_prefix}.grn.gamma'].detach().numpy())
                        )
                        layer.beta.assign(
                            tf.Variable(model_states[f'{stage_prefix}.grn.beta'].detach().numpy())
                        )
